...
The mean is pretty much in the center of the scale and the median is 6.

To get a better picture of this stats, we draw the box plot:

```{r echo=FALSE, message=FALSE, warning=FALSE, Boxplot}
boxplot(data = wine, x = wine$quality, range = 45/25)
        
```

The box plot shows the interquartile range (IQR) between 5 and 6 as well as the median, which is also falling right on top of the 75% quartile at 6. The whiskers mark the spots at 1.5 times IQR and reach to 4 at the lower end and 7 at the upper end. The remaining values at 3 and 8 are marked es outliers.
...
As expected the major amount of data points are around the IQR. From there it declines to the outer bounds quite regularly.
...
ploting volatile.acidity and citric.acid against quality:

```{r echo=FALSE, message=FALSE, warning=FALSE, VolAcid_CitAcid}
ggplot(data = wine) +
  geom_jitter(aes(x = quality, y = volatile.acidity), color = 'red', alpha = 1/2) +
